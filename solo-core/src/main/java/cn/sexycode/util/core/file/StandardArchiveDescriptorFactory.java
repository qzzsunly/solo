...
public class StandardArchiveDescriptorFactory implements ArchiveDescriptorFactory, JarFileEntryUrlAdjuster {
    private static final Logger log = LoggerFactory.getLogger(StandardArchiveDescriptorFactory.class);
    ...
    public ArchiveDescriptor buildArchiveDescriptor(URL url, String entry) {
        ...
        } else if (StringUtils.isEmpty(protocol) || "file".equals(protocol) || "vfszip".equals(protocol) || "vfsfile"
                .equals(protocol)) {
    ...
    public URL adjustJarFileEntryUrl(URL url, URL rootUrl) {
        final String protocol = url.getProtocol();
        final boolean check =
                StringUtils.isEmpty(protocol) || "file".equals(protocol) || "vfszip".equals(protocol) || "vfsfile"
                        .equals(protocol);
        if (!check) {
            return url;
        }
