...
public final class TypeResolver {
    ...
    public static Class<?>[] resolveRawArguments(Type genericType, Class<?> subType) {
        ...
        if (RESOLVES_LAMBDAS && subType.isSynthetic()) {
            Class<?> fi = genericType instanceof ParameterizedType && ((ParameterizedType) genericType)
    ...
    private static Type reify(Type genericType, final Map<TypeVariable<?>, Type> typeVariableMap,
        ...
        if (genericType instanceof ParameterizedType) {
            ...
            throw new UnsupportedOperationException(
                    "Attempted to reify wildcard type with name '" + wildcardType + "' which has " + upperBounds.length
                            + " upper bounds and " + lowerBounds.length + " lower bounds. "
                            + "Reification of wildcard types is only supported for "
                            + "the trivial case of exactly one upper bound and no lower bounds.");
        }
        throw new UnsupportedOperationException(
                "Reification of type with name '" + genericType.getTypeName() + "' and " + "class name '" + genericType
    ...
    private static Member getMemberRef(Class<?> type) {
        ...
        for (int i = getConstantPoolSize(constantPool) - 1; i >= 0; i--) {
            ...
            if (member == null || (member instanceof Constructor && member.getDeclaringClass().getName()
                    .equals("java.lang.invoke.SerializedLambda")) || member.getDeclaringClass().isAssignableFrom(type))
                continue;
